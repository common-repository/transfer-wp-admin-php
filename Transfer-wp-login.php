...
/*
 * Plugin Name: Transfer WP-ADMIN.php
 * Plugin URI:  https://www.wptechnical.in/transfer-wp-admin-php/
 * Description: Remove your WP-admin.php and wp-login.php quick  & Easy with Transfer wp-admin.php plugin. 
 * Version:     1.0
 * Author:      Neil G Patel, WPTechnical.in    
 * Author URI:  https://www.wptechnical.in/about-us/    
 * Text Domain: transfer-wp-login.php   
 * License:     GPLv2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 */

if ( defined( 'ABSPATH' ) && ! class_exists( 'transfer_WP_Login' ) ) {

	class transfer_WP_Login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'transfer_WP_Login', 'uninstall' ) );
		...
		public function wpmu_options() {
			echo (
				'<h3>' .
					_x( 'transfer wp-login.php', 'Text string for settings page', 'transfer-wp-login' ) .
				'</h3>' .
				'<p>' .
					__( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'transfer-wp-login' ) .
				'</p>' .
				'<table class="form-table">' .
					'<tr valign="top">' .
						'<th scope="row">' .
							__( 'Networkwide default', 'transfer-wp-login' ) .
						'</th>' .
						'<td>' .
		...
		public function admin_init() {
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'transfer-wp-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				echo (
					'<p>' .
						sprintf(
							__( 'To set a networkwide default, go to %s.', 'transfer-wp-login' ),
							'<a href="' . esc_url( network_admin_url( 'settings.php#rwl-page-input' ) ) . '">' .
								__( 'Network Settings', 'transfer-wp-login' ) .
							'</a>'
						) .
					'</p>'
				);
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'transfer-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}
		...
		public function plugins_loaded() {
			...
			load_plugin_textdomain( 'transfer-wp-login' );
			...
			if (
				...
				wp_die( __( 'This feature is not enabled.', 'transfer-wp-login' ), '', array( 'response' => 403 ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'transfer-wp-login' ), '', array( 'response' => 403 ) );
			}
